...
function BrowserifyCache(b, opts) {
  ...
  attachCachePersistHandler(b, cacheStore);
...
function attachCacheHooksToPipeline(b) {
  ...
  b.bundle = function(cb) {
    ...
    // TODO (jake): make this use .then() too
...
function invalidateCacheBeforeBundling(b, done) {
  ...
  // this upper scope stuff is kinda messed
  var hackyInvalidated;
  var hackyDeleted;
  var hackyError;

  return invalidateFilesPackagePaths(cache.filesPackagePaths)
    .then(() => invalidatePackageCache(cache.hashes, cache.packages))
    .then(() => invalidateCache(cache.hashes, cache.modules))
    .then(({invalidated, deleted}) => {
      hackyInvalidated = invalidated;
      hackyDeleted = deleted;
      return invalidateDependentFiles(cache, [].concat(invalidated, deleted));
    })
    .catch((err) => hackyError = err)
    .then(() => {
        b.emit('changedDeps', hackyInvalidated, hackyDeleted);
        done(hackyError, hackyInvalidated);
    });
}
...
function attachCachePersistHandler(b, cacheStore) {
